...
import math
import shutil
from socket import *  # bug1: import socket
...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_SAVE:
        ...
        if FIELD_SIZE not in json_data.keys():
            ...
            connection_socket.send(
                make_response_packet(OP_SAVE, 410, TYPE_FILE, f'This file "size" has to be included', {})) # bug2: 402 to 410
            return
        ...
        try:
            ...
            logger.info(f'<-- Upload plan: key {key}, total block number {total_block}, block size {block_size}.') #bug15: error to info
            connection_socket.send(
    ...
    if request_operation == OP_UPLOAD:
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is True:
            ...
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 402, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is completely uploaded.', {})) #bug3: 408 to 402
            return
        ...
        if block_index >= total_block:
            ...
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 405, TYPE_FILE, f'The "block_index" exceed the max index.', {})) #bug5: 410 to 405
            return
        if block_index < 0:
            logger.error(f'<-- The "block_index" should >= 0.')
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 406, TYPE_FILE, f'The "block_index" should >= 0.', {})) #bug6: 410 to 406
            return
    ...
    if request_operation == OP_DOWNLOAD:
        ...
        if block_index >= total_block:
            logger.error(f'<-- The "block_index" exceed the max index.')
            connection_socket.send(
                make_response_packet(OP_GET, 405, TYPE_FILE, f'The "block_index" exceed the max index.', {})) #bug7: 410 to 405
            return
        if block_index < 0:
            logger.error(f'<-- The "block_index" should >= 0.')
            connection_socket.send(
                make_response_packet(OP_GET, 406, TYPE_FILE, f'The "block_index" should >= 0.', {})) #bug8: 410 to 406
            return
...
def step_service(connection_socket, addr):
    ...
    while True:
        ...
        if request_type == TYPE_FILE:
            file_process(username, request_operation, json_data, bin_data, connection_socket) #bug9: data_process to file_process
            continue
...
def tcp_listener(server_ip, server_port):
    ...
    """
    global logger
    server_socket = socket(AF_INET, SOCK_STREAM)  #bug10: TCP: SOCK_STREAM
    server_socket.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
    server_socket.bind((server_ip, int(server_port)))
    server_socket.listen(20)
    logger.info(f'Server is ready')   #bug11: lack f''
    logger.info(
    ...
    while True:
        try:
            ...
            th = Thread(target=step_service, args=(connection_socket, addr))  #bug12: STEP_service to step_service
            th.daemon = True
            th.start()   #bug13: thread start
